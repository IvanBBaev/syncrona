...
module.exports = {
  forbidden: [
    ...
    },
    {
      name: "types-is-leaf",
      comment:
        "@syncro-now-ai/types is a pure leaf and must not depend on any other @syncro-now-ai package. NOTE: `types` has no src/ — its declarations live in packages/types/index.d.ts, so the `from` path matches the package root, not `/src`. (Reaches: packages/types/index.d.ts.)",
      severity: "error",
  ...
  options: {
    ...
    // source-resolvable edges (notably `@syncro-now-ai/types` -> index.d.ts) are
