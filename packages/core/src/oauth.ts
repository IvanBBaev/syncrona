// G1: optional ServiceNow OAuth 2.0 (Resource Owner Password Credentials grant).
//
// This is ADDITIVE — Basic auth (.env / credential store) stays the default.
// OAuth kicks in only when a client id + secret are configured; the same
// username/password are then exchanged at `oauth_token.do` for a Bearer token,
// which is refreshed on expiry or on a 401. Tokens are kept in memory per
// process (a CLI run re-acquires once); they are never written to disk.
